...
TFX_NAMESPACE_PACKAGES = [
    'tfx', 'tfx.*',
    'tfx.orchestration', 'tfx.orchestration.*'
]
...
ML_PIPELINES_SDK_PACKAGES = [
    ...
    'tfx.proto.orchestration', 'tfx.proto.orchestration.*',
    # TODO(XXX): Move `tfx.utils` to a location that emphasizes that
    # these are internal utilities.
    'tfx.utils', 'tfx.utils.*',
    # TODO(XXX): Move `Artifact` and `ComponentSpec` classes into `tfx.dsl.*`.
    'tfx.types', 'tfx.types.*',
]
